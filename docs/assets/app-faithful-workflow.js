(function () {
  ...
  //   0단계  • ▶ ▪ □ ○ ◦   가장 바깥
  //   1단계  ‣ ▸ ▹          그 아래, 앞 항목에 딸린 내용
  //   2단계  ※ *            덧붙이는 말
  //   3단계  - –            가장 안쪽, 잘게 쪼갠 것
  //
  // 이 차례는 한글파일에서 두 기호가 잇달아 나온 자리의 앞공백을 세어 정했습니다.
  // scripts/validate_marker_levels.py가 그 셈을 다시 해 이 표와 맞대어 봅니다.
  //   '▪' 다음의 '▸'가 안쪽 217곳 / 바깥 0곳
  //   '•' 다음의 '▸'가 안쪽 282곳 / 바깥 0곳
  //   '◦' 다음의 '※'가 안쪽 208곳 / 바깥 15곳
  const MARKER_LEVEL = {
    "•": 0, "▶": 0, "▪": 0, "□": 0, "○": 0, "◦": 0,
    "‣": 1, "▸": 1, "▹": 1,
    "※": 2, "*": 2,
  ...
  function sourceBlockMarkup(seat) {
    ...
    const CUSTOM_FRONT_BLOCK = /^c(0[2-9]|1[0-9])-w00-b1$/;
  ...
  function renderStep(work, step, requestedFaqNumber = "") {
    ...
    const customFrontStep =
      work.number === 0 &&
      (step.mainBlocks || []).some((one) => /^c(0[2-9]|1[0-9])-w00-b1$/.test(String(one.id || "")));
    const repeatsTitle =
    ...
    // 쪽 번호는 우선 쪽 지도(GUIDE_PDF_PAGES, 원문 PDF에서 업무별 시작 쪽을
    // 뽑아 둔 것)에서 이 업무의 쪽을 찾고, 없으면 옛 데이터의 work.pdfPages를
    // 씁니다. 둘 다 없으면(스캔본 편 등) 그냥 매뉴얼을 표지부터 엽니다.
    const sourceLink = byId("source-page-link");
    sourceLink.hidden = !data.downloads.manual;
    if (data.downloads.manual) {
      const pageMap = (window.GUIDE_PDF_PAGES || {})[activeChapter.id] || {};
      const firstPage =
        pageMap[work.id] || (Array.isArray(work.pdfPages) ? work.pdfPages[0] : null);
      if (firstPage > 0) {

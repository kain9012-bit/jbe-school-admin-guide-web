(function () {
  ...
  const routeFor = (workId, stepId) =>
    `#work=${encodeURIComponent(workId)}&step=${encodeURIComponent(stepId)}`;
  ...
  function relatedFaqsFor(work, step) {
  ...
  function renderFaqs(work, step) {
    const target = byId("related-faqs");
    const faqs = relatedFaqsFor(work, step);
      ...
      .map(
        (faq, index) => `
          <div class="accordion-item">
            <h3>
              <button class="btn-accordion" type="button" aria-expanded="false" aria-controls="faq-panel-${index}">
                <span>Q. ${escapeHtml(faq.question)}</span>
              </button>
            </h3>
            <div class="accordion-collapse" id="faq-panel-${index}" hidden>
  ...
  function renderStep(work, steps, activeIndex) {
  ...
  function renderWork(workId, stepId, focusPanel) {
    ...
    breadcrumb.innerHTML = `
      ...
      <li><a href="#overview">제1편 행정업무 및 보안</a></li>
      <li><span>${escapeHtml(work.title)}</span></li>
    `;

    renderStep(work, steps, activeIndex);
  ...
  function renderRoute() {
    ...
    if (!raw || raw === "overview" || raw === "downloads") {
      ...
      breadcrumb.innerHTML = `
        <li class="home"><a href="#overview">홈</a></li>
        <li><span>제1편 행정업무 및 보안</span></li>
      `;
      document.title = "학교행정업무 길라잡이 웹판 | 제1편 행정업무 및 보안";
      if (raw === "downloads") {
    ...
    renderWork(nextWorkId, nextStepId, focusPanel);
  }

  function formRoute(formId) {
    for (const work of data.sections) {
      const step = getSteps(work.id).find((candidate) => candidate.forms.includes(formId));
      if (step) return routeFor(work.id, step.id);
    }
    return "#overview";
  }
  ...
  function buildSearchIndex() {
    ...
    data.sections.forEach((work) => {
      ...
      index.push({
        ...
        route: routeFor(work.id, workflow.steps[0].id),
      ...
      workflow.steps.forEach((step) => {
        index.push({
          ...
          route: routeFor(work.id, step.id),
          text: [
            work.title,
            step.title,
            step.summary,
    ...
    data.faqs.forEach((faq) => {
      ...
      index.push({
        ...
        route: routeFor(matchingWork.id, matchingStep.id),
  ...
  function runSearch(query) {
    ...
    if (!trimmed) {
      searchStatus.textContent = "업무명이나 궁금한 문장을 입력하세요.";
      ...
      .map(
        ({ item }) => `
          <a class="search-result" href="${item.route}">
            <div class="search-result-meta"><span>${escapeHtml(item.type)}</span></div>
